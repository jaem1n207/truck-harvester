...
Chromium 기반 renderer로 Carmodoo page를 실제 브라우저처럼 렌더링하고, 2-up
sheet 단위 JPG bytes를 반환한다.
...
3. Chromium page에 승인된 Carmodoo URL을 직접 로드한다.
4. renderer 전용 print layout style과 bundled Korean font face를 주입한다.
...
Next.js route runtime은 `nodejs`로 둔다. Renderer 구현은 Playwright 기반이고,
`playwright`는 server route에서 dynamic import되는 runtime dependency다. Vercel
serverless 환경에서는 `@sparticuz/chromium`의 executable path와 launch args를
사용하고, local/dev 환경에서는 Playwright가 제공하는 Chromium을 사용한다.

serverless Chromium에는 CJK font가 포함되어 있지 않으므로 renderer는
`@fontsource/noto-sans-kr`의 400/700 woff2 파일을 읽어 data URL `@font-face`로
주입한다. Carmodoo 문서에는 renderer 전용 font family를 강제해 사용자의 OS나
Vercel image에 설치된 font에 의존하지 않는다.
...
자동 unit test만으로는 이 문제를 충분히 검증할 수 없다. 현재는
`e2e/carmodoo-render-font.spec.ts`와 `bun run test:carmodoo-render`가 실제
Chromium에서 renderer code path를 실행하고, Korean fixture JPG의 dark pixel 분포를
검사해 한글 glyph가 사라지지 않았음을 자동 확인한다. 실제 실패 URL 또는 fixture
HTML로 native screenshot 결과를 생성할 때는 다음을 확인한다.
...
6. GitHub Actions CI에서 `bun run test:carmodoo-render`를 실행해 PR마다 renderer
   font/glyph 회귀를 검증한다.
...
`bun run test:carmodoo-render`는 Playwright가 Next.js dev server를 띄우고 실제
Chromium을 실행한다. sandbox에서 port binding이 막히면 권한 상승 또는 일반 local
shell에서 다시 실행한다. `bun run build`도 Turbopack과 renderer dependency가
process나 port binding을 할 수 있으므로 sandbox에서 막히면 권한 상승으로 다시
실행한다.
