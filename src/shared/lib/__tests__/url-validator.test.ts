  ...
  describe('validateUrl', () => {
    ...
    it('허용되지 않은 도메인을 거부한다', () => {
      const result = validateUrl(
        'https://example.com/model/DetailView.asp?ShopNo=123&MemberNo=456&OnCarNo=789'
      )
    ...
    it('필수 파라미터가 누락된 URL을 거부한다', () => {
      const resultMissingShopNo = validateUrl(
        'https://www.truck-no1.co.kr/model/DetailView.asp?MemberNo=67890&OnCarNo=2025123456789'
      )
      expect(resultMissingShopNo.isValid).toBe(false)
      expect(resultMissingShopNo.error).toContain(
        '필수 파라미터가 누락되었습니다: ShopNo'
      )

      const resultMissingMemberNo = validateUrl(
        'https://www.truck-no1.co.kr/model/DetailView.asp?ShopNo=12345&OnCarNo=2025123456789'
      )
      expect(resultMissingMemberNo.isValid).toBe(false)
      expect(resultMissingMemberNo.error).toContain(
        '필수 파라미터가 누락되었습니다: MemberNo'
      )

      const resultMissingOnCarNo = validateUrl(
        'https://www.truck-no1.co.kr/model/DetailView.asp?ShopNo=12345&MemberNo=67890'
      )
      expect(resultMissingOnCarNo.isValid).toBe(false)
      expect(resultMissingOnCarNo.error).toContain(
        '필수 파라미터가 누락되었습니다: OnCarNo'
      )
    })

    it('잘못된 프로토콜을 거부한다', () => {
      const result = validateUrl(
        'ftp://www.truck-no1.co.kr/model/DetailView.asp'
      )
      expect(result.isValid).toBe(false)
  ...
  describe('getValidUrls', () => {
    it('유효하고 중복되지 않은 URL만 반환한다', () => {
      const urlResults = [
        ...
        {
          url: 'https://www.truck-no1.co.kr/model/DetailView.asp?ShopNo=54321&MemberNo=09876&OnCarNo=2024987654321',
          isValid: true,
          isDuplicate: false,
        },
      ]
